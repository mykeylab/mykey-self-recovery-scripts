...
const TransferGasPrice = '16.000001' // Gwei
const TransferGas = 250000

let MykeyAccount = process.env.MykeyAccountAddress
let ToAccount = process.env.ToAccountAddress
let TokenAmount = web3.utils.toWei('0', 'ether');

(async () => {

	let tokenBalance = await web3.eth.getBalance(MykeyAccount)
	TokenAmount = tokenBalance
	console.log(`MykeyAccount ${MykeyAccount}, eth balance ${TokenAmount}`)
	if (TokenAmount == 0) {
	...
	}, [MykeyAccount, ToAccount, TokenAmount]);
	...
	console.log('account:', MykeyAccount);
	console.log('sig:', sig);
